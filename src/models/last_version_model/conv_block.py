...
class ConvBlockSingleInput(nn.Cell):
    ...
    def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.ReLU, group=1,
        ...
        self.conv2 = nn.Conv2d(med_planes, med_planes, kernel_size=3, stride=stride, group=group) # padding 0 -> 1 , pad_mode='pad', padding=1
        self.bn2 = norm_layer(med_planes)
        self.act2 = act_layer()  # no inplace
    ...
    def construct(self, x):
        ...
        x = self.conv2(x)  #double is x + x_t
        x = self.bn2(x)
        if self.drop_block is not None:
            x = self.drop_block(x)
        x2 = self.act2(x)
...
class ConvBlockDoubleInput(nn.Cell):
    ...
    def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.ReLU, group=1,
        ...
        self.conv2 = nn.Conv2d(med_planes, med_planes, kernel_size=3, stride=stride, group=group)   # , pad_mode='pad', padding=1
        self.bn2 = norm_layer(med_planes)
        self.act2 = act_layer()
