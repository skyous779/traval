...
class EvaluateCallBack(Callback):
    ...
    def epoch_end(self, run_context):
        ...
        if result["acc"] > self.best_acc:
            ...
            self.save_best_path = os.path.join(self.src_url, "best.ckpt")
            ms.save_checkpoint(cb_params.train_network, self.save_best_path)

        print("epoch: %s acc: %s, best acc is %s" %
              (cb_params.cur_epoch_num, result["acc"], self.best_acc), flush=True)
        # if cb_params.cur_epoch_num > self.total_epochs * 0.5:
        cur_epoch_num = cb_params.cur_epoch_num
        if args.run_openi or args.run_modelarts : #forget the openi
            try:
